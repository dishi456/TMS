...
import { rateTenant, type FormState } from "./actions";
import { FieldLabel, inputClass, btn } from "@/components/ui";

const CRITERIA: { name: string; label: string }[] = [
  { name: "rentDiscipline", label: "Rent Payment Discipline" },
  { name: "propertyMaintenance", label: "Property Maintenance" },
  { name: "communication", label: "Communication Skills" },
  { name: "ruleCompliance", label: "Rule Compliance" },
  { name: "conduct", label: "Overall Conduct" },
];

export function RateTenantForm({
  leaseId,
  defaults,
}: {
  leaseId: string;
  ...
  const [state, formAction, pending] = useActionState<FormState, FormData>(rateTenant, undefined);
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <label className="flex flex-col gap-1">
        ...
        <textarea name="feedback" rows={3} defaultValue={defaults?.feedback} className={inputClass} placeholder="How was this tenant?" />
      </label>

      <label className="flex items-center gap-2 text-sm text-slate-700">
        <input type="checkbox" name="recommend" defaultChecked={defaults?.recommend} className="h-4 w-4" />
        I would recommend this tenant to other landlords
      </label>
