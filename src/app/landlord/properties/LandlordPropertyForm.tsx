    ...
    <form action={formAction} className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="flex flex-col gap-1">
          ...
          <select name="availability" defaultValue={defaults.availability ?? "AVAILABLE"} className={inputClass}>
            {AVAIL.map((a) => <option key={a} value={a}>{a.charAt(0) + a.slice(1).toLowerCase()}</option>)}
          </select>
        </label>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <label className="flex flex-col gap-1">
          ...
          <input name="rentAmount" type="number" min="0" required defaultValue={defaults.rentAmount} className={inputClass} />
        </label>
        <label className="flex flex-col gap-1">
          <FieldLabel>Deposit ($)</FieldLabel>
          <input name="securityDeposit" type="number" min="0" defaultValue={defaults.securityDeposit ?? "0"} className={inputClass} />
        </label>
        <label className="flex flex-col gap-1">
          <FieldLabel>Number of units</FieldLabel>
          <input name="numberOfUnits" type="number" min="1" defaultValue={defaults.numberOfUnits ?? "1"} className={inputClass} />
        </label>
        <label className="flex flex-col gap-1">
      ...
      <label className="flex flex-col gap-1">
        <FieldLabel>Other amenities (comma-separated)</FieldLabel>
        <input name="amenities" defaultValue={defaults.amenities} placeholder="Gym, Swimming Pool, Garden" className={inputClass} />
      </label>
      ...
      <div className="rounded-lg border border-blue-100 bg-blue-50/60 p-3">
        <label className="flex items-center gap-2 text-sm font-medium text-slate-800">
          ...
          🌐 Show this property on the public listings page
        </label>
        <p className="mt-1 text-xs text-slate-500">Goes live only after Master Admin approval. Untick to keep it private (won&apos;t appear on /listings).</p>
      </div>
