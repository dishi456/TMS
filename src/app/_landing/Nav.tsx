...
export function Nav({ variant = "overlay", account = null }: { variant?: "overlay" | "solid"; account?: string | null }) {
  ...
  const navLink = "whitespace-nowrap rounded-full px-3.5 py-2 text-[15px] font-bold text-slate-700 transition-colors hover:bg-blue-100 hover:text-blue-700";
        ...
        <div className="animate-fade-up max-h-[80vh] overflow-y-auto border-t border-slate-200 bg-white/95 px-5 py-4 backdrop-blur-lg xl:hidden">
          <div className="flex flex-col gap-1">
            ...
            {account ? (
              ...
              <Link href="/login" onClick={() => setOpen(false)} className="mt-2 rounded-lg border border-slate-300 px-3 py-2.5 text-center text-sm font-medium text-slate-700 hover:bg-slate-50">Sign in</Link>
            )}
          </div>
        </div>
      )}
