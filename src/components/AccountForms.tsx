...
import { updateOwnProfile, changeOwnPassword, type AccountState } from "@/app/actions/account";
...
export function AccountProfileForm({
  ...
  // `extended` adds a username + location/currency preferences (landlord); the
  // admin account page leaves it off for a lean name + phone form.
  extended?: boolean;
}) {
  const [state, formAction, pending] = useActionState<AccountState, FormData>(updateOwnProfile, undefined);
  return (
    <form action={formAction} className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        {extended && <label className="flex flex-col gap-1"><FieldLabel>Username</FieldLabel><input name="username" defaultValue={defaults.username} placeholder="your_handle" className={inputClass} /></label>}
...
export function AccountPasswordForm() {
  const [state, formAction, pending] = useActionState<AccountState, FormData>(changeOwnPassword, undefined);
  return (
